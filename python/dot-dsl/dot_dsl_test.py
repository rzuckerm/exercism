...
class DotDslTest(unittest.TestCase):
    ...
    def test_graph_with_attributes(self):
        ...
        self.assertEqual(g.nodes, [Node("a", {"color": "green"}), Node("c", {}), Node("b", {"label": "Beta!"})])
        self.assertEqual(g.edges, [Edge("b", "c", {}), Edge("a", "b", {"color": "blue"})])
    ...
    def test_malformed_EDGE(self):  # pylint: disable=invalid-name
        with self.assertRaises(ValueError) as err:
            Graph([(EDGE, 1, 2)])
        self.assertEqual(type(err.exception), ValueError)
