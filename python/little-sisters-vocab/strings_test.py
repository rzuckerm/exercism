import unittest
import pytest
from strings import add_prefix_un, make_word_groups, remove_suffix_ness, adjective_to_verb
...
class LittleSistersVocabTest(unittest.TestCase):
    ...
    def test_adjective_to_verb(self):
        ...
        for variant, (sentence, index, expected) in enumerate(
            ...
            with self.subTest(
                f"variation #{variant}", sentence=sentence, index=index, expected=expected
            ):
