...
class SetsTest(unittest.TestCase):
    ...
    def test_clean_ingredients(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                ...
                self.assertEqual(
                    clean_ingredients(item[0], item[1]), (result[1], result[2]), msg=error_msg
                )
    ...
    def test_check_drinks(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                f"variation #{variant}", iputs="all drinks", results="drinks classified"
            ):
    ...
    def test_categorize_dish(self):
        test_data = zip(
            sorted(recipes_without_duplicates, reverse=True)[::3], dishes_categorized[::3]
        )

        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                f"variation #{variant}", inputs="all recipes list", results="categorized dishes"
            ):
    ...
    def test_compile_ingredients(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                ...
                error_message = (
                    "Expected a proper set of combined ingredients, but something went wrong."
                )
    ...
    def test_separate_appetizers(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(
                f"variation #{variant}", inputs="dishes with appetizers", results="appetizers only"
            ):

                error_message = (
                    "Expected only appetizers returned, but some dishes remain in the group."
                )
