Function Get-Products() {
    ...
    $Start, $End = ($Largest) ? @($Max, $Min) : @($Min, $Max)
    $result = $null
    $factors = @()
    foreach ($i in $Start..$End) {
        ...
        foreach ($j in $i..$End) {
            $n = $i * $j
            if (-not $result -or ($Largest ? ($n -ge $result) : ($n -le $result))) {
                $anyChange = $true
                if (($s = [string]$n) -eq (-join $s[$s.Length..0])) {
                    if ($n -ne $result) { $factors = @() }
                    $result = $n
                    $factors += ($i -lt $j) ? @(,@($i, $j)) : @(,@($j, $i))
                }
            }
