Function Invoke-Grep() {
    ...
    $matchPattern = ($Insensitive) ? $Pattern.ToLower() : $Pattern
    foreach ($file in $Files) {
        $lineNum = 0
        foreach ($line in (Get-Content -Path $file)) {
            $lineNum++
            $matchLine = ($Insensitive) ? $line.ToLower() : $line
            $match = ($Whole) ? ($matchLine -ceq $matchPattern) : ($matchLine -cmatch $matchPattern)
            if ($match -xor $Invert) {
                if ($List) {
                    Write-Output $file
                    break
                ...
                Write-Output ((($Files.Count -gt 1) ? "${file}:" : "") + (($Number) ? "${lineNum}:" : "") + $line)
            }
        }
    }
}
